...
DataAccessObject.create = function(data, options, cb) {
  ...
  if (Array.isArray(data)) {
    ...
    async.map(data, function(item, done) {
      const idValue = getIdValue(Model, item);

      if (idValue) {
        Model.findById(idValue, options, function(err, model) {
          if (err) return cb(err);
          if (!model) {
            // return cb(errorModelNotFound(idValue));
            self.create(item, options, function(err, result) {
              // Collect all errors and results
              done(null, {err: err, result: result || item});
            });
          } else {
            model.updateAttributes(item, options, function(err, result) {
              // Collect all errors and results
              done(null, {err: err, result: result || item});
            });
          }
